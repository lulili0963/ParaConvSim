...
from src.mi_systems.rewriter import T5Rewriter, T5FeedbackRewriter
...
for p in os.listdir('4extremerepeated'):
  if os.path.splitext(p)[1]=='.json':
    ...
    base_path = f"data/f_moreround_fewshot_combinedex_round1_4extremerepeated/para_generated_conversations_{os.path.splitext(p)[0]}"
    ...
    pipeline = RecursivePipeline([
        ...
        GPT3FeedbackProvider(f"4extremerepeated/{p}", "instructions.json")
    ])
